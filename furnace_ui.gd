...
# IMPORTANTE: El orden aquí define quién es el Slot 0, 1 y 2
# Slot 1: Entrada (0), Slot 2: Carbón (1), Slot 3: Resultado (2)
...
func _on_slot_clicked(index):
	...
	# INDEX 2 es SIEMPRE el resultado (Slot3 en el array corregido)
	if index == 2:
		if furnace_slot["item"] != null:
			# Intentamos meter el producto en la hotbar usando tu función
			var exito = player_hotbar.recolect(furnace_slot["atlas"], furnace_slot["item"])
			
			if exito:
				# Solo si hubo espacio en la hotbar, vaciamos el slot del horno
				current_furnace.items[index] = {"item": null, "amount": 0, "atlas": null}
				update_slots()
			else:
				print("Inventario lleno, no puedes recoger eso")
		return

	# Lógica de intercambio normal para Slot1 y Slot2
	...
	# Usamos call_deferred para evitar el error de "locked"
